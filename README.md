...
## Overview
Scalable, multicamera distributed system for realtime pointcloud stitching at [IQR Lab](https://iqr-lab.github.io/). This program is currently designed to use **D400 Series Intel RealSense** depth cameras. Using the [librealsense 2.0 SDK](https://github.com/IntelRealSense/librealsense), depth frames are grabbed and pointclouds are computed on the edge, before sending the raw XYZRGB values to a central node over TCP sockets. The central program stitches the pointclouds together and displays a visualization with a [PCL](http://pointclouds.org/) viewer.
...
1. Build and compile the cetral node binaries.
  ```
